...
class SignInActivity : AppCompatActivity() {
    ...
    private fun signIn() {
        ...
        //On récupère le documents dans la collection USERS qui a pour
        //attributs les informations rentrées par l'utilisateur
        databse.collection(Constants.KEY_COLLECTION_USERS)
            .whereEqualTo(Constants.KEY_EMAIL, binding.emailRegister.text.toString())
            .whereEqualTo(Constants.KEY_PASSWORD, binding.passwordRegister.text.toString())
            .get()
            .addOnCompleteListener {
                //Lorsqu'on a fini la recherche  :
